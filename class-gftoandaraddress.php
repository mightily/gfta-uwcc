...
class GFtoAndarAddress extends GFAddOn {

	protected $_version = GF_SIMPLE_FIELD_ADDON_VERSION;
	protected $_min_gravityforms_version = '1.9';
	protected $_slug = 'gftoandaraddress';
	protected $_path = 'gftoandaraddress/gftoandaraddress.php';
	protected $_full_path = __FILE__;
	protected $_title = 'Gravity Forms to Andar Address';
	protected $_short_title = 'Andar Address Field';
	...
	public function pre_init() {
		...
		if ( $this->is_gravityforms_supported() && class_exists( 'GF_Field' ) ) {
			require_once( 'includes/class-andar-address-field.php' );
		}
	}
...
//	}
	...
	public function tooltips( $tooltips ) {
		$simple_tooltips = array(
			'input_class_setting' => sprintf( '<h6>%s</h6>%s', esc_html__( 'Input CSS Classes', 'gftoandar' ), esc_html__( 'The CSS Class names to be added to the field input.', 'gftoandar' ) ),
		);
		...
		if ( $position == 250 ) {
			...
			<li class="input_class_setting field_setting">
				<label for="input_class_setting">
					<?php esc_html_e( 'Input CSS Classes', 'gftoandar' ); ?>
